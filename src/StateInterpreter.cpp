    ...
    //		double distanceGoalKeeperBall = Math::distancePoint(goalRobot.getSelfState().position, state.ball.getPosition());
                ...
                if(attackerRobot.getRobotStrategyBase().isBlocked())
            {
                ...
                cout<<"pos Defender: x: "<<defenderRobot.getSelfState().position.x<<endl<<" y "<<defenderRobot.getSelfState().position.y<<endl;
                ...
                if(state.ball.position.x < vss::MAX_COORDINATE_X/2 &&
                  ...
                  (defenderRobot.getSelfState().position.y > 60)&& (defenderRobot.getSelfState().position.y < 70) && defenderRobot.getSelfState().position.x > 35 )
